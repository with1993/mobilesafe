	...
	public void onClick(View v) {
		...
		case R.id.tv_uninstall:
			 if(mAppInfo.isSystem){
	        	 ToastUtils.show(getApplicationContext(), "此应用为系统应用，不能卸载！");
	         }else{
		...
		case R.id.tv_start:
			...
			if(launchIntentForPackage!=null){
				...
				ToastUtils.show(getApplicationContext(), "此应用不能打开！");
			}
			break;
		case R.id.tv_share:
			Intent intent = new Intent(Intent.ACTION_SEND);
			intent.setType("text/plain");
			intent.putExtra(Intent.EXTRA_TEXT, "分享一个应用，应用名为"+mAppInfo.getPackageName());
			startActivity(intent);
			break;
		}
